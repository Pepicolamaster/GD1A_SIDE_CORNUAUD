...
public class MovementTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (jumpCount < 1)
        {
            ...
            if (Input.GetButtonDown("Jump") && !isCrouching &&!isDead && hasDoubleJump)
            {
                //on assigne au Y du rigidbody le jumpingPower définit
                rb.velocity = new Vector2(rb.velocity.x, jumpingPower);
                jumpCount += 1;
    ...
    private void OnTriggerEnter2D(Collider2D collision) //savoir si le joueur peut se relever
    {
    ...
    {
        if(leave.gameObject.tag == "forceCrouch")
        {
            canStand = true;
        }
    }
